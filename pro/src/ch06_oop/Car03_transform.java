...
public class Car03_transform {
	...
	//model 초기화생성자
	Car03_transform(String model){
		//model = cmodel;
		//Car03(cmodel,"yellow");//error
		this(model,"green");//this는 생성자에서 또 다른 생성자를 호출(원래 생성자는 다른 생성자를 호출불가하지만 this를 사용하면 가능하다
	}
	
	//model,color 초기화생성자
	Car03_transform(String model, String color){
		this(model,color,500); //생성자호출할때는 반드시 생성자의 첫번째문장에서 호출해야 작동함!
	
	}
	
	//model,color,maxSpeed 초기화생성자
	Car03_transform(String model, String color, int maxSpeed){
		this.model = model;
		this.color = color ;
